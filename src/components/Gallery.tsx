  ...
  return (
    <section id="gallery" className="py-16 sm:py-24 bg-[#1A0E14] relative overflow-hidden bg-suits-dark-watermark border-b border-[#F3C4A0]/15">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* ── Section Header - Split Placement (Title on Left, Filters on Right) ── */}
          ...
          {filteredAlbums.map((album) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative p-5 z-10 translate-y-0 sm:translate-y-2 sm:group-hover:translate-y-0 transition-transform duration-300">

                  {/* Date pill — electric blue accent */}
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-2.5"
                    style={{ background: '#3B66FF', boxShadow: '0 2px 10px rgba(59,102,255,0.5)' }}>
                    <span className="text-[9px] font-black uppercase text-white tracking-widest">{album.date}</span>
                  </div>

                  {/* Title — large, bold, editorial */}
                  <h3
                    className="text-white font-black uppercase leading-[1.1] group-hover:text-[#EEF2FF] transition-colors"
                    style={{
                      fontFamily: "'Plus Jakarta Sans', 'Bebas Neue', sans-serif",
                  ...
                  {/* Open album CTA */}
                  <div className="flex items-center gap-2 mt-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 translate-y-0 sm:translate-y-2 sm:group-hover:translate-y-0">
                    <span className="w-6 h-6 rounded-full bg-white flex items-center justify-center shrink-0">
                      <Maximize2 className="w-3 h-3 text-[#3B66FF]" />
                    </span>
                    <span className="text-[11px] font-bold uppercase text-[#EEF2FF]" style={{ letterSpacing: '0.08em' }}>
                      Ouvrir l'album · {album.photos.length} photos
                    </span>
                  </div>
                </div>
      ...
      {/* ── FULL MULTI-PHOTO ALBUM VIEWER MODAL ── */}
      {activeAlbum && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6"
          style={{ background: 'rgba(8,4,6,0.97)', backdropFilter: 'blur(20px)' }}
          onClick={closeAlbum}
        >
          {/* Close Button */}
          <button
            onClick={closeAlbum}
            className="absolute top-3 sm:top-5 right-3 sm:right-5 p-2 sm:p-2.5 rounded-full text-[#F3C4A0] hover:text-white hover:bg-[#B93A34] transition-all z-20"
            style={{ border: '1px solid rgba(243,196,160,0.2)', background: 'rgba(43,20,32,0.8)' }}
          >
          ...
          <button
            ...
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 rounded-full text-[#F3C4A0] hover:bg-[#B93A34] transition-all z-20"
            style={{ background: 'rgba(43,20,32,0.8)', border: '1px solid rgba(243,196,160,0.2)' }}
          >
          ...
          <button
            ...
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 rounded-full text-[#F3C4A0] hover:bg-[#B93A34] transition-all z-20"
            style={{ background: 'rgba(43,20,32,0.8)', border: '1px solid rgba(243,196,160,0.2)' }}
          >
          ...
          <div
            className="max-w-5xl w-full flex flex-col items-center gap-3 sm:gap-5 my-auto px-6 sm:px-0"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header info bar */}
            <div className="w-full text-center space-y-1">
              <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full bg-[#2B1420] text-[#F3C4A0] border border-[#F3C4A0]/20 text-[10px] sm:text-[11px] font-black uppercase tracking-wider mb-1">
                <span>{activeAlbum.category}</span>
                <span>•</span>
                <span>{activeAlbum.date}</span>
              </div>

              <h3 className="font-black uppercase text-[#F5EDE4] text-base sm:text-2xl md:text-3xl tracking-tight line-clamp-1">
                {activeAlbum.title}
              </h3>

              <p className="text-[11px] sm:text-xs font-bold text-[#F3C4A0] uppercase tracking-widest">
                Photo {photoIndex + 1} sur {activeAlbum.photos.length}
              </p>
            </div>

            {/* Active Large Image Display */}
            <div
              className="relative rounded-2xl overflow-hidden shadow-2xl w-full flex items-center justify-center bg-black/40"
              style={{ border: '1px solid rgba(243,196,160,0.2)', maxHeight: '55vh', minHeight: '220px' }}
            >
              <img
              ...
              {/* Caption Overlay */}
              <div className="absolute bottom-0 inset-x-0 p-2.5 sm:p-3 bg-gradient-to-t from-black/90 to-transparent text-center">
                <p className="text-[11px] sm:text-xs font-bold text-[#F5EDE4] line-clamp-2">
            ...
            <div className="w-full flex items-center justify-start sm:justify-center gap-2 sm:gap-3 overflow-x-auto py-2 px-2 sm:px-4 scrollbar-none">
              {activeAlbum.photos.map((photo, i) => (
                <button
                  key={photo.id}
                  onClick={() => setPhotoIndex(i)}
                  className={`relative rounded-xl overflow-hidden shrink-0 transition-all duration-300 border-2 ${
                    i === photoIndex
                      ? 'border-[#B93A34] scale-105 opacity-100 shadow-lg shadow-[#B93A34]/50'
